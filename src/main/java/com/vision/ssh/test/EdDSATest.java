...
public class EdDSATest {
	...
	public static KeyPair getKeyPair(final String algorithm) {
		...
		try {
			...
			// throws: NoSuchAlgorithmException: no such algorithm: Ed25519 for provider
			// EdDSA
	...
	public static void test4() {
		...
		try {
			String publicKeyBytesAsString = new String(publicKeyBytes, charsetName);
			String privateKeyBytesAsString = new String(privateKeyBytes, charsetName);

			System.out.println(publicKeyBytesAsString);
			System.out.println(privateKeyBytesAsString);

			// String decodedToUTF8 = new
			// String(publicKeyBytesAsString.getBytes("ISO-8859-1"), "UTF-8");

			byte[] publicKeyBytesAsStringToBytes = publicKeyBytesAsString.getBytes(charsetName);
			byte[] privateKeyBytesAsStringToBytes = privateKeyBytesAsString.getBytes(charsetName);

			// for (int i = 0; i < publicKeyBytes.length; i++) {
			// System.out.println(publicKeyBytes[i] + " " +
			// publicKeyBytesAsStringToBytes[i]);
			// }
			// System.out.println(Arrays.equals(publicKeyBytes,
			// publicKeyBytesAsStringToBytes));
			//
			// for (int i = 0; i < privateKeyBytes.length; i++) {
			// System.out.println(privateKeyBytes[i] + " " +
			// privateKeyBytesAsStringToBytes[i]);
			// }
			// System.out.println(Arrays.equals(privateKeyBytes,
			// privateKeyBytesAsStringToBytes));

			KeyPair keyPair = SshdConnector.getKeyPair(publicKeyBytesAsStringToBytes, privateKeyBytesAsStringToBytes,
					algorithm);
